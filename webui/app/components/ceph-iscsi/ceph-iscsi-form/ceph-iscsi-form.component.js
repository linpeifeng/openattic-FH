...
class CephIscsiForm {
  ...
  generateTargetId () {
    return "iqn.2017-10.com.suse:" + Date.now();
  }
  ...
  resolveModel (res) {
    ...
    this.model.images.forEach((image) => {
      ...
      // 加载完数据之后，记下图片的初始数据 readipos， writeipos
      image.oldReadIOPS = image.settings.readiops;
      image.oldWriteIOPS = image.settings.writeiops;
      image.oldiopsInfo = image.settings.setiopsinfo;
    });
  ...
  submitAction () {
    console.log("this.model.images", this.model.images);
    let requestModel = this.buildRequest();
    // for 循环 requestModel.images
    // 如果 image.settings.oldReadIOPS !== image.settigs.readiops 
    // requestModel.Readxxx = true
    // 如果 image.settings.oldReadIOPS !== image.settigs.readiops 
    // requestModel.Writexxx = true
