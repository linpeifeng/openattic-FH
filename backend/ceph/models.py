...
class CephRbd(NodbModel, RadosMixin):  # aka RADOS block device
    ...
    @bulk_attribute_setter(['num_objs', 'obj_size', 'size', 'data_pool_id',
                            'features', 'old_format', 'block_name_prefix', 'parent'])
...
class CrushmapVersion(NodbModel):
    ...
    def get_tree(self):
        ...
        #logger.error('crushmap = %s', crushtree)
        try:
           crushmap_filepath = "/etc/ceph/crushmap.json"
           if os.path.exists(crushmap_filepath) and os.access(crushmap_filepath, os.R_OK):
               with open(crushmap_filepath) as data_file:
        ...
        for device in self.crushmap["devices"]:
